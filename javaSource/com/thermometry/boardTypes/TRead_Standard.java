...
public class TRead_Standard {

	private static final Logger sLogger = Logger.getLogger(TRead_Standard.class.getName());
	...
	private double[] fVoltages = new double[numberOfChannels];
	...
	public void read(byte[] packetArraybytes, Calibrator calibrators[]){
		...
		//Print Values
		System.out.println(" ----------------- TRead_Standard VALUES -----------------  ");
		...
		System.out.println(" ----------------- TRead_Standard RAW VALUES -----------------  ");
	...
	public void CalcReducedData(Calibrator calibrators[])
	{
		...
		if (readMode == TRead_Standard)           // DT-470 diode mode
		{
	...
	private void readChannels(){
		...
		for (int j = 0; j < numberOfChannels; j++) {
			...
			ADac[j] = (float) (ADacRaw[j] * 0.007833);
	...
	private void CalcReducedResistanceModes(Calibrator calibrators[]) {
		...
		if (readMode == TRead_Standard)             // Standard mode
		{
			// includes 2x100k + protection resistors + Rmux
			Rbridge = 220100.0;    // Total bridge resistance in series with thermometer
